#!/bin/bash
# 応答品質ゲート (Stop hook)
# 直近のユーザー入力より後ろにある assistant text を連結し、以下のパターンを検知したら
# exit 2 で応答を再生成させる。
#
...
#  15  見出し・表のセル・箇条書きが文で終わる
#      構造行の末尾が「です」「ます」「する」「ない」等の述語
#      根拠: 表のセル・見出し・箇条書きの項目は体言止めか名詞句で書く
#  16  表のセルと本文の重複
...
# 7) text の前処理 (以降のパターン共通): コードブロック / バッククォート / 引用括弧内を除外
# 説明文中で hook 検知キーワードを引用した場合の誤検知を防ぐ
...
if printf '%s' "$filtered_text" | grep -qE '叩き[[:space:]]*v[0-9]+|ひな形|△[[:space:]]|要確認'; then
  if printf '%s' "$filtered_text" | grep -qE '完了|起票済|終わり|報告(?!書)|まとめました'; then
...
# 発火は 1 語から。同一語を何回使っても 1 語と数える設定では、同じ語の反復を素通りさせる。
declare -a fluff=(
...
# 対象外: 「フロー」(ワークフロー)、「系」、「軸」、「集約」(ログ集約)、「圧縮」(gzip 等)、
#         「ベース」(データベース / ナレッジベース) は誤検知が多いため入れない。
...
# 検査範囲は直近ユーザー入力以降の全 assistant text の合算のため、ツールを挟む長い応答では
# 送付文面 1 通より厳しめに働く。
...
if [[ ${#promise_hits[@]} -gt 0 ]]; then
  ...
  violations+=("空約束・自己表現の宣言: [$joined] (宣言ではなく仕組み・実行結果を書く)")
fi
...
# (完了報告の過去形は、指示済み作業の報告と区別できないため対象にしない)。
# 検査対象は「最後のテキスト入力」のため、ターン途中に質問が割り込むと指示済み作業の
# 宣言でも発火し得る (許容する。バイパスはユーザーの明示指示で)。
interrog_re='[??]|でしょうね|でしょうか|ですか|ますか|ませんか|っけ|のかな'
imper_re='してください|して下さい|しなさい|してね|しといて|しとけ|お願いします|お願いね|頼む|やって|直して|直せ|作って|作れ|書いて|書け|追記して|反映して|進めて|講じて|消して|削除して|戻して|更新して|修正して|実行して|write-go'
if [[ -n "${user_text:-}" ]] \
...
# 10-G) パターン 11: 自分ラベルでの報告
# 応答内で作った整理用の呼び名 (通1 / 2通目 等) で成果物を指す書き方を検知する。
# 読み手の画面にある実名 (ファイル名等) で書く。検知は誤検知の少ない形に限定する。
...
if [[ -n "$self_label_hits" ]]; then
  ...
  violations+=("自分ラベルでの報告: [$joined] (Output Style「Concise JA」「成果物報告の型」。読み手の画面にある実名で書く)")
fi

# 10-H) パターン 12: 逃げの締め (指示の丸投げ / 自己卑下の約束)
# 調査を広げた末に「指示をください」で締める、叱責に「これ以上〜しません」で締める、の
# 2 型を止める。承認依頼の文言 (「可否をください」「[change-go」) は対象外。
# 引用「」内は filtered_text で除外済み。
...
if [[ -n "$handoff_hits" ]]; then
  ...
  violations+=("逃げの締め: [$joined] (rules「start-approval」「作業の継続」。次の一手を自分で決めて示す。約束でなく仕組みと実行結果を書く)")
fi

# 10-I) パターン 13: 送付文面の 1 文が 60 字超
# ユーザー入力が文面の依頼のときだけ動かす (資料からの引用に反応させない)。応答内の
# 文面行 (> / ※ / ■ / ・ 始まり) を「。」で区切って数える。URL を含む文は除外。
...
# 指摘は矛盾の指摘であり、理由なしに合わせる応答は矛盾を 1 つ増やす。
...
if [[ -n "$struct_bad" ]]; then
  ...
  violations+=("見出し・表のセル・箇条書きが文で終わる: [$joined] (Output Style「Concise JA」。体言止めか名詞句で書く)")
fi
...
if [[ ${#banned_hits[@]} -gt 0 ]]; then
  ...
  violations+=("禁止前置き・充足判断・文案評価断定フレーズ: [$joined] (文体規則違反)")
fi
...
{
  echo "[応答品質ゲート違反]"
  echo "以下のパターンを検知しました。応答を再生成してください:"
  echo ""
  for v in "${violations[@]}"; do
    echo "  - $v"
  done
  echo ""
  echo "バイパス (緊急時のみ、${principal}の明示指示で): 次のユーザーメッセージに [hook-bypass: response-quality] を含める"
} >&2
