...
#
# 通過条件:
#   - 禁止語・内部リポジトリパス・絶対パス・ローカル拡張子・組版記号・外部 AI ツール言及・
#     長い識別子の繰り返しのいずれも含まない
#
# 検査範囲: Write は全文、Edit/MultiEdit は追加・置換後の文字列のみ。
# 既存本文全体は検査しないため、送付直前に全文対象の grep チェックを別途行う。
#
# バイパス:
#   - 直前のユーザーメッセージに [hook-bypass: draft-precheck] がある場合
#
...
if [[ -n "$transcript" && -f "$transcript" ]]; then
  ...
  if printf '%s' "$last_user_msg" | grep -q '\[hook-bypass: draft-precheck\]'; then
    exit 0
  fi
fi
...
if printf '%s' "$content" | grep -qE "$internal_paths"; then
  matched=$(printf '%s' "$content" | grep -nE "$internal_paths" | head -3)
  hits="${hits}  - 内部リポジトリパス検出:"$'\n'"${matched}"$'\n'
fi
if printf '%s' "$content" | grep -qE "$abs_paths"; then
  matched=$(printf '%s' "$content" | grep -nE "$abs_paths" | head -3)
  hits="${hits}  - 絶対パス検出:"$'\n'"${matched}"$'\n'
fi
...
#    Edit/MultiEdit は追加分のみが対象 (全文の判定は送付前チェックで行う)。
#    バッククォート内は数えない。
