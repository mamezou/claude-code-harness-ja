...
#   種別ごとの直近の検出例 / 検出語の頻度 / 常時ロード指示の行数
# ログの形式: <ISO時刻>\t<種別: [検出語] (根拠)>|<種別...>  ([regen-skip] 付きは通過記録)
...
norm=$(grep -av '^[[:space:]]*$' "$log" | awk -F'\t' '
  NF >= 2 {
    ts=$1; body=$2; skip=0
    if (body ~ /^\[regen-skip\] /) { skip=1; sub(/^\[regen-skip\] /, "", body) }
...
echo "- 件数: ${cur_n} 件 (前期間 ${prev_n} 件)。うち再生成ループ防止で通過した記録 ${cur_skip} 件"
echo
